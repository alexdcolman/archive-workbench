...
**Estado actualizado:** 2026-09-01 · **versión:** 0.89.0 RC83
...
- **OPS-01-TABS:** validar que RC83 recupere el cambio de pestaña con un solo clic, mantenga respuesta prácticamente inmediata y no reintroduzca reruns globales. **Criterio de cierre:** un clic cambia la pestaña en Windows CPU y las interacciones pasivas no reinician la vista;
- **OPS-01-COPY:** validar la adopción de una copia completa recibida dentro de un proyecto vacío sin salir de Archive Workbench ni extraer carpetas manualmente. **Criterio de cierre:** ZIP verificado -> confirmación -> backup -> copia activa en la misma carpeta; un proyecto con trabajo rechaza el reemplazo automático y remite a paquete de cambios;
- **OPS-01-STOP:** validar el cierre comprensible del servicio administrado. **Criterio de cierre:** `Cerrar Archive Workbench` termina sólo la instancia propia, libera el puerto y no detiene otros procesos o contenedores;
