...
## Capacidades previstas después del núcleo

Quedan registradas para etapas posteriores:

1. **Sugerencias automáticas de entidades** por objeto o documento, siempre revisables y sin creación automática de autoridades aceptadas.
2. **Material audiovisual local**, con registro, transcripción segmentada y revisión mediante el modelo textual común.
3. Descarga desde YouTube u otras plataformas como plugin opcional, no como dependencia del núcleo.
4. Comparación sistemática de Docling, Tesseract y Surya.
5. Validación de CUDA y posible aislamiento de backends con dependencias incompatibles.
