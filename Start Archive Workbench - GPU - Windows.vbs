...
logPath = fso.BuildPath(temp, "archive_workbench_gpu_start_" & Replace(CStr(Timer), ".", "_") & ".log")
shell.Environment("PROCESS")("AW_GUI_LAUNCH") = "1"
command = "cmd.exe /d /c """ & fso.BuildPath(base, "Start Archive Workbench - GPU - Windows.bat") & """ >""" & logPath & """ 2>&1"
rc = shell.Run(command, 0, True)
If rc <> 0 Then
  content = "No se pudo iniciar Archive Workbench con GPU."
